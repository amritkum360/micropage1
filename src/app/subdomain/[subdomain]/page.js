...
export default function SubdomainWebsitePage({ params }) {
  ...
  console.log('🌐 Subdomain Page - Subdomain:', subdomain);

  const loadWebsiteBySubdomain = useCallback(async () => {
    try {
      console.log('🔍 Loading website for subdomain:', subdomain);
      
      // Call API to get website by subdomain
      const response = await fetch(`/api/websites/subdomain/${subdomain}`);
      
      if (!response.ok) {
        if (response.status === 404) {
          console.log('❌ Website not found for subdomain:', subdomain);
          setNotFound(true);
          return;
        }
      ...
      console.error('❌ Failed to load website by subdomain:', error);
  ...
  useEffect(() => {
    if (subdomain) {
      loadWebsiteBySubdomain();
    }
  }, [loadWebsiteBySubdomain, subdomain]);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            The website for <strong>{subdomain}.jirocash.com</strong> could not be found.
          </p>
